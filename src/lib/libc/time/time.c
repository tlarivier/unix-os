/*
 * time.c — libc time wrappers: time/gettimeofday/clock_gettime/nanosleep
 * plus the derived sleep/usleep/clock helpers.
 *
 * Invariants:
 *  - Each wrapper is a thin _syscall trampoline over one __NR_* number.
 *  - CLOCKS_PER_SEC == 100 matches the kernel HZ tick frequency.
 *  - All time values are user-space copies; no shared state with the kernel.
 *
 * Not allowed:
 *  - Spinning or busy-waiting (sleep/usleep go through nanosleep).
 *  - Locale-aware formatting (no strftime here).
 */
