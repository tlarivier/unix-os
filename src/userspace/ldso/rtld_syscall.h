...
static inline int rtld_open(const char *path, int flags, int mode) {
  ...
  __asm__ volatile("int $0x80"
                   : "=a"(ret)
                   : "a"(__NR_open), "b"(path), "c"(flags), "d"(mode));
  return ret;
}
...
static inline int rtld_lseek(int fd, int off, int whence) {
  ...
  __asm__ volatile("int $0x80"
                   : "=a"(ret)
                   : "a"(__NR_lseek), "b"(fd), "c"(off), "d"(whence));
  return ret;
}

static inline void *rtld_mmap(void *addr, size_t len, int prot, int flags,
                              int fd, int off) {
  void *ret;
  (void)off; /* offset not used for MAP_ANONYMOUS */
  __asm__ volatile("int $0x80"
                   : "=a"(ret)
                   : "a"(__NR_mmap), "b"(addr), "c"(len), "d"(prot), "S"(flags),
                     "D"(fd));
  return ret;
}
