/*
 * rtld.c — userspace dynamic linker (/lib/ld.so): reads the main ELF via auxv,
 * mmaps and relocates DT_NEEDED shared objects (R_386_{32,PC32,GLOB_DAT,
 * RELATIVE,COPY,JMP_SLOT}), resolves symbols via the ELF hash table, runs
 * ctors, then jumps to AT_ENTRY.
 *
 * Invariants:
 *  - No libc dependency — uses only inline int $0x80 syscalls from
 * rtld_syscall.h.
 *  - Up to MAX_LIBS=32 libs loaded; next_lib_base grows monotonically.
 *  - PLT0 is patched with __rtld_resolve so lazy binding falls back to the
 * resolver.
 *  - All relocations are eagerly resolved at load time (lazy entry is dead code
 * unless a lib skips a reloc).
 *
 * Not allowed:
 *  - Calling libc symbols (libc is the thing being relocated).
 *  - Allocating from a heap (only mmap of fixed-size lib slots).
 */
...
static void parse_dynamic(loaded_lib_t *lib) {
  ...
  for (Elf32_Dyn *dyn = lib->dynamic; dyn->d_tag != DT_NULL; dyn++) {
    switch (dyn->d_tag) {
    case DT_SYMTAB:
      lib->symtab = (Elf32_Sym *)(lib->base + dyn->d_val);
      break;
    case DT_STRTAB:
      lib->strtab = (const char *)(lib->base + dyn->d_val);
      break;
    case DT_HASH:
      lib->hash = (uint32_t *)(lib->base + dyn->d_val);
      break;
    case DT_REL:
      lib->rel = (Elf32_Rel *)(lib->base + dyn->d_val);
      break;
    case DT_RELSZ:
      lib->relsz = dyn->d_val;
      break;
    case DT_JMPREL:
      lib->pltrel = (Elf32_Rel *)(lib->base + dyn->d_val);
      break;
    case DT_PLTRELSZ:
      lib->pltrelsz = dyn->d_val;
      break;
    case DT_PLTGOT:
      lib->pltgot = (uint32_t *)(lib->base + dyn->d_val);
      break;
    case DT_INIT:
      lib->init = (void (*)(void))(lib->base + dyn->d_val);
      break;
    case DT_FINI:
      lib->fini = (void (*)(void))(lib->base + dyn->d_val);
      break;
    case DT_BIND_NOW:
      bind_now = 1;
      break;
...
static loaded_lib_t *load_library_internal(const char *path, int depth) {
  ...
  for (int i = 0; i < ehdr.e_phnum; i++) {
    ...
    if (rtld_read(fd, &phdr, sizeof(phdr)) != (int)sizeof(phdr))
      continue;
    ...
    if (phdr.p_type == PT_LOAD) {
      ...
      if (phdr.p_filesz > 0) {
        ...
        while (remaining > 0) {
          uint32_t chunk =
              remaining > sizeof(read_buf) ? sizeof(read_buf) : remaining;
  ...
  parse_dynamic(lib);
  if (load_dependencies(lib, depth) < 0)
    return NULL;
  if (process_relocations(lib) < 0)
    return NULL;
...
static loaded_lib_t *setup_main_executable(Elf32_Phdr *phdr, uint32_t phnum,
                                           uint32_t base) {
  if (num_libs >= MAX_LIBS)
    return NULL;
...
void _start_c(uint32_t *sp) {
  ...
  if (at_phdr && at_phnum) {
    main_lib =
        setup_main_executable((Elf32_Phdr *)at_phdr, at_phnum, prog_base);
  }
  ...
  void (*entry)(int, char **, char **) =
      (void (*)(int, char **, char **))at_entry;
